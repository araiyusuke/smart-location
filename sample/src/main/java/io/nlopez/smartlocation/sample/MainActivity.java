...
public class MainActivity extends Activity implements OnLocationUpdatedListener, OnActivityUpdatedListener, OnGeofencingTransitionListener {
    ...
    private void showLast() {
        ...
        if (detectedActivity != null) {
            // 現在のユーザーの行動の状態を判定してくれる機能。静止、徒歩、自転車、自動車を判定してくれます。
    ...
    private void startLocation() {
        // 権限チェックが完了して、ここからはじまる
        provider = new LocationGooglePlayServicesProvider();
        // 位置情報の設定確認
        provider.setCheckLocationSettings(true);

        // SmartLocationの内部クラスにstaticなBuilderクラスがある。
        SmartLocation smartLocation = new SmartLocation.Builder(this).logging(true).build();

        // 位置情報取得方法によってproviderを使い分ける
        // LocationControl.start provider.start
        smartLocation.location(provider).start(this); //onLocationUpdated)

        // ユーザの行動状態を取得(クルマとかバイク)
        smartLocation.activity().start(this); // onActivityUpdated
